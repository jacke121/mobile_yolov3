...
def train():
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        batch_size=config.batch_size, shuffle=True, num_workers=32, pin_memory=True)

    # net and optimizer
    net = mobile_yolo.Mobile_YOLO(config, is_training=True)
    optimizer = _get_optimizer(config, net)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=config.decay_step, gamma=config.decay_gamma)
